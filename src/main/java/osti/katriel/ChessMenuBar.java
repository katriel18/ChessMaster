...
public class ChessMenuBar extends JMenuBar {
    ...
    private void aboutHandler() {
        JOptionPane.showMessageDialog(this.getParent(), "YetAnotherChessGame v1.0 by:\nBen Katz\nMyles David\n"
    ...
    /**
     * Takes an appropriate action if the exit button is clicked. Uses Tony
     * Allevato's code for exiting a GUI app without System.exit() calls.
     */
    ...
    private void toggleGraveyardHandler() {
        ((ChessPanel) this.getParent()).getGraveyard(1)
                .setVisible(!((ChessPanel) this.getParent()).getGraveyard(1).isVisible());
        ((ChessPanel) this.getParent()).getGraveyard(2)
                .setVisible(!((ChessPanel) this.getParent()).getGraveyard(2).isVisible());
    }
    ...
    private void toggleGameLogHandler() {
        ...
        if (((ChessPanel) this.getParent()).getGameLog() instanceof ChessGameLog) {

            ((ChessGameLog) ((ChessPanel) this.getParent()).getGameLog())
                    .setVisible(!((ChessGameLog) ((ChessPanel) this.getParent()).getGameLog()).isVisible());
